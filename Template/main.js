...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const buttonCloseCart = document.getElementById("btnCloseCart");
...
function goNextMobile() {
  ...
  if (Math.abs(defaultTransformMobile) >= slider.scrollWidth / 1.7)
    defaultTransformMobile = 0;
...
prevMobile.addEventListener("click", () => {
  goPrevMobile();
  if (defaultTransformMobile === 0)
    return prevMobile.classList.add("invisible");
});
...
function goNextKeyboard() {
  ...
  if (Math.abs(defaultTransformKeyboard) >= slider.scrollWidth / 1.7)
    defaultTransformKeyboard = 0;
...
prevKeyboards.addEventListener("click", () => {
  goPrevKeyboard();
  if (defaultTransformKeyboard === 0)
    return prevKeyboards.classList.add("invisible");
});
...
function goNextMonitor() {
  ...
  if (Math.abs(defaultTransformMonitor) >= slider.scrollWidth / 1.7)
    defaultTransformMonitor = 0;
...
prevMonitor.addEventListener("click", () => {
  goPrevMonitor();
  if (defaultTransformMonitor === 0)
    return prevMonitor.classList.add("invisible");
});
...
showPasswordCheckBox.addEventListener("change", (e) => {
  ...
  hiddenPassword.textContent = e.target.checked
    ? "Ẩn mật khẩu"
    : "Hiện mật khẩu";
});
...
const openModalButtonsResign = document.querySelectorAll(
  '[data-modal-target="signup-modal"]'
);
const closeModalButtonsResign = document.querySelectorAll(
